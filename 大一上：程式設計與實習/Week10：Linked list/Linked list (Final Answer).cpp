// 完成更改-來自紀錄 (From 11/18 Badcode.cpp)
...
// OPERATION
    ...
    // 先接 a&b節點 前方節點指向本身的next
    if(previous_a == NULL){
        head = current_b; // 注意不會改到main裡面的head，是用『return head』的方式copy過去
    }else{
    ...
    // 再接 a&b節點 本身指向後方的next -> 直接互換
    Node *tmp = current_a->next;
